...
interface StudentChatsProps {
  user: any;
}

export function StudentChats({ user }: StudentChatsProps) {
  ...
  const fetchUniversityProfile = useCallback(async (universityId: string) => {
    setLoadingProfile(true);
    setUniversityProfile(null);
    try {
      const { data: profileData } = await supabase
        .from('profiles')
        .select('university_name, location, contact_email, contact_phone, description, website_url, logo_url, programs')
        .eq('user_id', universityId)
        .maybeSingle();
      
      if (profileData) {
        setUniversityProfile(profileData);
      } else {
        setUniversityProfile(null);
      }
    } catch (err) {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    if (data && data.length > 0) {
      const universityIds = data.map(c => c.university_id);
      const { data: profiles } = await supabase
        .from('profiles')
        .select('user_id, university_name, logo_url')
        .in('user_id', universityIds);
      ...
      const convsWithUnread = await Promise.all(
        (data as any[]).map(async (c) => {
          ...
          const profile = profileMap.get(c.university_id);
          return { 
            ...c, 
            university_name: profile?.university_name || 'University',
  ...
  useEffect(() => {
    ...
    fetchUniversityProfile(selectedConv.university_id);
      ...
      <div className="flex gap-0 bg-gray-900 border border-gray-800 rounded-[2.5rem] shadow-[0_8px_30px_rgba(0,0,0,0.5)] overflow-hidden flex-1 min-h-0" style={{ height: 'calc(100vh - 200px)' }}>
              ...
              filteredConvs.map((conv) => (
                ...
                >
                  <div className={`w-12 h-12 rounded-[1rem] flex items-center justify-center flex-shrink-0 font-bold text-base border shadow-inner transition-colors ${
                    ...
                    {(conv.university_name?.[0] || conv.university_name?.[0] || 'S').toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2 min-w-0">
                        <PresenceIndicator userId={conv.student_id} />
                        <p className={`font-bold text-sm truncate ${selectedConv?.id === conv.id ? 'text-white' : 'text-gray-300'}`}>{conv.university_name || conv.university_name}</p>
                      </div>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800 bg-black shadow-sm z-10">
              <div className="flex items-center gap-4">
                ...
                <div className="w-12 h-12 rounded-[1rem] bg-gray-800 text-white border border-gray-700 flex items-center justify-center font-bold text-base flex-shrink-0 shadow-inner">
                  {(selectedConv.university_name?.[0] || selectedConv.university_name?.[0] || 'S').toUpperCase()}
                </div>
                <div>
                  <div className="flex items-center gap-2.5 mb-0.5">
                    <p className="font-bold text-white text-base tracking-wide">{selectedConv.university_name || selectedConv.university_name}</p>
                    <PresenceIndicator userId={selectedConv.student_id} showText={true} />
                  </div>
                  <p className="text-xs text-gray-500 font-medium">{selectedConv.university_name}</p>
                </div>
              </div>
              <button
        ...
        <AnimatePresence>
          {showProfilePanel && selectedConv && (
            ...
            >
              <div className="flex flex-col h-full bg-gray-900/50 backdrop-blur-md border-l border-gray-800 transition-all duration-300">
                  <div className="flex items-center justify-between p-6 border-b border-gray-800 bg-black/40">
                    <h3 className="font-bold text-white tracking-tight flex items-center gap-2">
                      <User className="w-5 h-5 text-gray-500" /> UNIVERSITY PROFILE
                    </h3>
                ...
                <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                  {loadingProfile ? (
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="flex flex-col items-center text-center pt-2">
                        <div className="w-24 h-24 rounded-3xl bg-black border border-gray-800 text-white flex items-center justify-center font-bold text-3xl mb-4 shadow-inner">
                          {(universityProfile.university_name?.[0] || selectedConv.university_name?.[0] || 'U').toUpperCase()}
                        </div>
                        <h4 className="font-bold text-white text-xl tracking-wide">{universityProfile.university_name || selectedConv.university_name}</h4>
                        {universityProfile.location && (
                          <p className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-gray-500 mt-2">
                            <MapPin className="w-3.5 h-3.5" /> {universityProfile.location}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="space-y-4 bg-black rounded-2xl p-5 border border-gray-800 shadow-inner">
                        {universityProfile.contact_email && (
                          <div className="flex items-center gap-3 text-sm">
                            <div className="p-2 bg-gray-900 rounded-lg border border-gray-800"><Mail className="w-4 h-4 text-gray-400" /></div>
                            <span className="text-gray-300 font-medium break-all">{universityProfile.contact_email}</span>
                          </div>
                        )}
                        {universityProfile.contact_phone && (
                          <div className="flex items-center gap-3 text-sm">
                            <div className="p-2 bg-gray-900 rounded-lg border border-gray-800"><Phone className="w-4 h-4 text-gray-400" /></div>
                            <span className="text-gray-300 font-medium">{universityProfile.contact_phone}</span>
                          </div>
                        )}
                      </div>

                      {/* Academics */}
                      {universityProfile.programs && universityProfile.programs.length > 0 && (
                        <div className="bg-black/60 rounded-2xl p-5 border border-gray-800 shadow-inner">
                          <h5 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-4">Available Programs</h5>
                          <div className="flex flex-wrap gap-2">
                            {universityProfile.programs.map((prog: string, i: number) => (
                              <span key={i} className="px-3 py-1 bg-gray-900 text-gray-300 text-xs font-medium rounded-lg border border-gray-800">
                                {prog}
                              </span>
                            ))}
                          </div>
                      ...
                      {universityProfile.description && (
                        <div className="bg-black/60 rounded-2xl p-5 border border-gray-800 shadow-inner">
                          <h5 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">About University</h5>
                      ...
                      {/* Website */}
                      {universityProfile.website_url && (
                        <a 
                          href={universityProfile.website_url} 
                          target="_blank" 
                          rel="noopener noreferrer" 
                          className="flex items-center justify-between p-4 bg-gray-900 rounded-2xl border border-gray-800 hover:bg-gray-800 hover:border-gray-700 transition-colors group"
                        >
                          <div className="flex items-center gap-3">
                            <Globe className="w-5 h-5 text-gray-400 group-hover:text-white transition-colors" />
                            <span className="text-sm font-bold text-gray-300 group-hover:text-white transition-colors">Visit Website</span>
                          </div>
                    ...
                    <div className="text-center py-12 bg-black rounded-2xl border border-gray-800 m-4">
                      ...
                      <p className="text-sm text-gray-400 font-medium px-4">University profile data is incomplete or unavailable.</p>
                    </div>
                  )}
                </div>
              </div>

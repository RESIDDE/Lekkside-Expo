        ...
        <div className="relative flex flex-col max-h-[90vh]">
          ...
          <div className="p-8 pb-0">
            ...
            <div className="p-4 bg-muted/30 border border-border/40 rounded-2xl flex gap-3 mb-6">
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest leading-none">Standard Payload</p>
          ...
          <div className="flex-1 overflow-y-auto px-8 space-y-6 pb-8 custom-scrollbar">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between px-1">
                <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Active Custom Fields</span>
                <span className="text-[10px] font-semibold bg-primary/10 text-primary px-2 py-0.5 rounded-full">{fields.length} Configured</span>
              </div>
              ...
              <div className="space-y-2">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          {field.required && (
                            <span className="text-[10px] bg-destructive/10 text-destructive font-semibold px-1.5 py-0.5 rounded uppercase tracking-tighter">Required</span>
                          )}
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="flex items-center gap-1.5 text-[10px] font-semibold text-muted-foreground/60 bg-muted/40 px-2 py-0.5 rounded-full uppercase tracking-widest">
                ...
                {fields.length === 0 && !isAddingField && (
                  <div className="py-12 border-2 border-dashed border-border/40 rounded-3xl flex flex-col items-center justify-center text-center px-6">
                    ...
                    <p className="text-sm font-semibold text-muted-foreground/60">No custom fields defined</p>
                  </div>
                )}
              </div>
            </div>
              ...
              {isAddingField ? (
                ...
                >
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="field-label" className="text-[10px] font-semibold uppercase tracking-widest text-primary px-1">Label Name</Label>
                      <Input
                        id="field-label"
                        value={newField.label || ""}
                        onChange={(e) => setNewField((prev) => ({ ...prev, label: e.target.value }))}
                        placeholder="e.g., Job Title"
                        className="h-11 rounded-xl bg-white border-border/40 font-semibold"
                      />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="field-type" className="text-[10px] font-semibold uppercase tracking-widest text-primary px-1">Input Style</Label>
                      <Select
                        value={newField.type}
                        onValueChange={(value) => setNewField((prev) => ({ ...prev, type: value as CustomField["type"] }))}
                      >
                        <SelectTrigger className="h-11 rounded-xl bg-white border-border/40 font-semibold">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent className="rounded-xl border-border/40 shadow-premium">
                          <SelectItem value="text" className="rounded-lg font-semibold uppercase text-[10px] tracking-widest py-3">Text Input</SelectItem>
                          <SelectItem value="textarea" className="rounded-lg font-semibold uppercase text-[10px] tracking-widest py-3">Large Text</SelectItem>
                          <SelectItem value="select" className="rounded-lg font-semibold uppercase text-[10px] tracking-widest py-3">Dropdown</SelectItem>
                          <SelectItem value="checkbox" className="rounded-lg font-semibold uppercase text-[10px] tracking-widest py-3">Option Check</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                  ...
                  {newField.type === "select" && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-2">
                      <Label htmlFor="field-options" className="text-[10px] font-semibold uppercase tracking-widest text-primary px-1">Choices (Comma Separated)</Label>
                      <Input
                        id="field-options"
                        value={optionsInput}
                  ...
                  <div className="flex items-center justify-between p-3 bg-white/50 rounded-xl border border-primary/10">
                    <div className="space-y-0.5">
                      <Label htmlFor="field-required" className="text-[11px] font-semibold text-foreground">Mandatory Field</Label>
                  ...
                  <div className="flex gap-2 pt-2">
                    <Button onClick={handleAddField} className="h-12 flex-1 rounded-2xl bg-primary hover:bg-primary/90 text-white font-semibold uppercase tracking-widest shadow-lg shadow-primary/10 gap-2">
                      Add to Form
                    ...
                    <Button
                      ...
                      className="h-12 rounded-2xl font-semibold text-muted-foreground hover:bg-muted"
                    >
                      Discard
                    </Button>
                  </div>
                ...
                <Button
                  variant="outline"
                  className="w-full h-14 rounded-2xl border-dashed border-2 border-primary/20 hover:border-primary/40 hover:bg-primary/5 text-primary/70 font-semibold uppercase tracking-widest gap-2 transition-all group"
          ...
          <div className="p-8 bg-muted/20 border-t border-border/40 flex items-center justify-between">
            <Button
              ...
              className="h-12 px-6 rounded-2xl font-semibold text-muted-foreground"
            >
              Go Back
            </Button>
            <Button
              onClick={handleSave}
              disabled={isSaving}
              className="h-12 px-10 rounded-2xl bg-foreground hover:bg-foreground/90 text-white font-semibold uppercase tracking-widest shadow-xl shadow-foreground/20 gap-3"
            >

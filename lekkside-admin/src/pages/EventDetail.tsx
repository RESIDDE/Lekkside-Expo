...
export default function EventDetail() {
  ...
  const handleUndoCheckIn = useCallback(async (guestId: string) => {
    try {
      ...
      toast({ title: 'Check-in undone', description: 'Guest check-in has been reversed.' });
    } catch {
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      ...
      toast({ title: 'Event deleted', description: 'The event and all its guests have been deleted.' });
      navigate('/events');
    } catch {
  ...
  const handleDeleteImportedData = async () => {
    ...
    try {
      ...
      toast({ title: 'Data deleted', description: 'All imported guest data has been removed.' });
    } catch {
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center py-20 text-center px-4">
          ...
          <p className="text-muted-foreground mt-2 max-w-md">This event might have been moved or deleted from our servers.</p>
          <Button 
            variant="outline" 
    ...
    <AppLayout>
      ...
      >
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="icon" className="h-11 w-11 rounded-xl border-border/50 text-destructive hover:bg-destructive/10 hover:text-destructive">
        ...
        <motion.div variants={itemVariants} className="relative group">
          <div className={cn(
            ...
            event.image_url ? "border-transparent" : "border-dashed border-border/50"
          )}>
            {event.image_url ? (
              <img 
                src={event.image_url} 
              ...
              <div className="w-full h-full flex flex-col items-center justify-center text-muted-foreground/40 gap-4">
                ...
                <p className="font-heading font-bold text-xl uppercase tracking-widest">No cover image</p>
              </div>
            )}
            ...
            <div className="absolute inset-x-0 bottom-0 p-8 bg-gradient-to-t from-black/60 to-transparent flex justify-end">
              ...
              >
                {isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Pencil className="w-4 h-4" />}
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 premium-card p-8 flex flex-col items-center justify-center relative overflow-hidden group">
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-[10px] font-extrabold text-muted-foreground uppercase tracking-widest mt-1">Completion</span>
              </div>
            </div>

            <div className="mt-8 grid grid-cols-3 gap-8 w-full">
              <div className="text-center space-y-1">
                <p className="text-2xl font-heading font-black text-foreground">{stats.total}</p>
                <p className="text-[10px] font-extrabold text-muted-foreground uppercase tracking-widest">Total Guests</p>
              </div>
              <div className="text-center space-y-1">
                <p className="text-2xl font-heading font-black text-[hsl(var(--success))]">{stats.checkedIn}</p>
                <p className="text-[10px] font-extrabold text-muted-foreground uppercase tracking-widest">Arrived</p>
              </div>
              <div className="text-center space-y-1">
                <p className="text-2xl font-heading font-black text-[hsl(var(--warning))]">{stats.pending}</p>
                <p className="text-[10px] font-extrabold text-muted-foreground uppercase tracking-widest">Remaining</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 premium-card p-8 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="outline" className="rounded-lg bg-muted/30 border-border/50 font-bold px-3 py-1">
            ...
            <div className="space-y-6 max-h-[240px] overflow-y-auto pr-2 custom-scrollbar">
              {stats.ticketTypes && Object.keys(stats.ticketTypes).length > 0 ? (
                Object.entries(stats.ticketTypes).map(([type, data], idx) => {
                  const percent = data.total > 0 ? (data.checkedIn / data.total) * 100 : 0;
                  return (
                    <div key={type} className="space-y-2 group">
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-bold text-foreground group-hover:text-primary transition-colors">{type}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-muted-foreground">
                            {data.checkedIn} <span className="opacity-50">/</span> {data.total}
                          </span>
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center opacity-50">
                  ...
                  <p className="text-sm font-medium">No ticket type data available</p>
                </div>
              )}
            </div>
          </div>
        ...
        <motion.div variants={itemVariants} className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl font-heading font-black text-foreground">Guest Management</h2>
                <p className="text-sm text-muted-foreground font-medium">Manage and check-in your attendees</p>
              </div>
            </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="w-full sm:w-auto p-1.5 h-auto bg-muted/30 rounded-[1.5rem] border border-border/40 grid grid-cols-3 sm:flex gap-1">
              {[
                ...
                { id: 'pending', label: 'Remaining', color: 'bg-[hsl(var(--warning))]' },
                { id: 'checked-in', label: 'Arrived', color: 'bg-[hsl(var(--success))]' }
              ].map((tab) => (
                <TabsTrigger 
                  key={tab.id}
                  value={tab.id} 
                  className={cn(
                    ...
                    tab.id === 'pending' && "data-[state=active]:bg-[hsl(var(--warning))]",
                    tab.id === 'checked-in' && "data-[state=active]:bg-[hsl(var(--success))]"
                  )}
                >
                  <span className="relative z-10 flex items-center gap-2">
                    {tab.label}
                    <Badge variant="secondary" className="bg-white/20 text-white border-0 text-[10px] font-black h-5 px-1.5 min-w-[20px] rounded-md">
                      {tab.id === 'all' ? stats.total : (tab.id === 'pending' ? stats.pending : stats.checkedIn)}
                    </Badge>
                  </span>
                </TabsTrigger>
              ))}
              ...
              <AnimatePresence mode="popLayout">
                {guestsLoading ? (
                  ...
                  >
                    ...
                    {searchQuery ? (
                      <>
                        <h3 className="text-xl font-heading font-bold text-foreground">No matching guests</h3>
                        <p className="text-muted-foreground mt-2 max-w-sm">We couldn't find anyone matching "{searchQuery}". Check the spelling or try another term.</p>
                        <Button 
                          variant="ghost" 
                      ...
                      <>
                        <h3 className="text-xl font-heading font-bold text-foreground">Your guest list is empty</h3>
                        <p className="text-muted-foreground mt-2 max-w-sm">Import your attendees from a CSV or Excel file to begin management.</p>
                        <div className="mt-8">
                          <ImportDialog eventId={event.id} />
                        </div>
                      </>
                      ...
                      <>
                        <h3 className="text-xl font-heading font-bold text-foreground">
                          {activeTab === 'pending' ? 'All checked in! 🎉' : 'No arrivals yet'}
                        </h3>
      ...
      <AlertDialog open={deleteDialogType === 'data'} onOpenChange={(open) => !open && setDeleteDialogType(null)}>
        <AlertDialogContent className="rounded-[2rem] border-border/40 p-8">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl font-heading font-bold">Clear All Data?</AlertDialogTitle>
            <AlertDialogDescription className="text-base text-muted-foreground">
              This will permanently remove all <span className="font-bold text-foreground">{stats.total} guests</span> from this event. The event profile will be preserved, but all check-in history and metadata will be lost.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8 gap-3">
            <AlertDialogCancel className="h-12 rounded-2xl border-border/50 font-bold">Cancel</AlertDialogCancel>
            <AlertDialogAction
      ...
      <AlertDialog open={deleteDialogType === 'event'} onOpenChange={(open) => !open && setDeleteDialogType(null)}>
        <AlertDialogContent className="rounded-[2rem] border-border/40 p-8">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-2xl font-heading font-bold">Delete Event permanently?</AlertDialogTitle>
            <AlertDialogDescription className="text-base text-muted-foreground leading-relaxed">
              This action cannot be undone. You are about to delete <span className="font-bold text-foreground">"{event.name}"</span> and all its associated data, including <span className="font-bold text-foreground">{stats.total} guest records</span>.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8 gap-3">
            <AlertDialogCancel className="h-12 rounded-2xl border-border/50 font-bold">Keep Event</AlertDialogCancel>
            <AlertDialogAction
